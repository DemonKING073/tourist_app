  ...
  void _addHotel() async {
    if (_formKey.currentState!.validate()) {
      ...
      for (final imagePath in _selectedImages) {
        final imageUrl =
            await RecommendedService().uploadHotelImage(File(imagePath));
        if (imageUrl != null) {
          imageUrls.add(imageUrl);
        }
      }
      final placeId =
          await RecommendedService().getPlaceIdByPlaceName(_placeName.text);
      ...
      if (placeId != null) {
        ...
        await RecommendedService().createHotel(hotel, placeId).then((value) {
          if (value != null) {
            ESnackBar.showSuccess(context, 'Successfully added hotel');
          } else {
            ESnackBar.showError(context, 'Failed to add hotel');
          }
        });
      } else {
        print('Error: Unable to find place with name $_placeName');
      }
